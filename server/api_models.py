...
# ---------------------------------------------------------------------------
# Admin index overview models
# ---------------------------------------------------------------------------


class AdminIndexRootInfo(BaseModel):
    """A single indexed root in the admin index overview.

    One row per *root* the in-memory manifest knows about.  The
    ``watcher_active`` flag is sourced from
    :class:`WatchService.is_watching` (the manifest's
    ``RootManifest.watching`` field is currently a dataclass
    default and not a reliable source).  ``last_job_id`` is the
    most-recent background job whose ``result.root`` matches the
    root path, useful for jumping from a root row to the
    corresponding job in the ``jobs`` list.
    """

    root: str = Field(description="Root path or label.")
    total_files: int = Field(ge=0, description="Number of files indexed under this root.")
    total_chunks: int = Field(ge=0, description="Number of chunks indexed under this root.")
    watcher_active: bool = Field(
        default=False,
        description=(
            "``True`` if a file watcher is currently active for this "
            "root.  Sourced from ``WatchService.is_watching``; "
            "``False`` when the watch service is unavailable or the "
            "root is not being watched."
        ),
    )
    last_job_id: str | None = Field(
        default=None,
        description=(
            "Identifier of the most recent background job for this "
            "root, or ``None`` when no job has touched the root in "
            "this process."
        ),
    )


class AdminIndexJobInfo(BaseModel):
    """A single background indexing job in the admin index overview.

    The most recent jobs (capped at 20) are surfaced here so the CMS
    can render a job history without a secondary call.  ``errors``
    is ``None`` (not an empty list) when the job completed cleanly,
    matching :class:`BackgroundJobService.list_jobs`.
    """

    job_id: str = Field(description="Unique job identifier.")
    status: str = Field(description="Current job status (e.g. ``queued``, ``running``, ``completed``, ``failed``).")
    queued_at: str | None = Field(default=None, description="ISO 8601 timestamp when the job was queued.")
    started_at: str | None = Field(default=None, description="ISO 8601 timestamp when the job started.")
    completed_at: str | None = Field(default=None, description="ISO 8601 timestamp when the job completed.")
    result: dict[str, Any] | None = Field(
        default=None,
        description="Result payload from a completed job.",
    )
    errors: list[str] | None = Field(
        default=None,
        description="Error messages if the job failed.",
    )


class AdminIndexFileInfo(BaseModel):
    """A single indexed file in the admin index overview.

    One row per ``(root, file_path)`` pair from the in-memory
    manifest.  ``language`` is detected from the file extension via
    :meth:`FileScanner.language_for`; ``None`` for unrecognised
    extensions.  ``has_summary_embedding`` is computed against the
    live corpus (not from the manifest) so the flag always reflects
    the actual in-memory chunks.
    """

    root: str = Field(description="Root namespace containing this file.")
    file_path: str = Field(description="Relative file path within the root.")
    chunk_count: int = Field(ge=0, description="Number of chunks for this file.")
    language: str | None = Field(
        default=None,
        description=(
            "Detected programming language (e.g. ``\"python\"``, "
            "``\"typescript\"``, ``\"markdown\"``), or ``None`` when "
            "the file extension is not in the scanner's known set."
        ),
    )
    last_indexed_at: str | None = Field(
        default=None,
        description="ISO 8601 timestamp of the last index operation for this file.",
    )
    has_summary_embedding: bool = Field(
        default=False,
        description=(
            "``True`` if any chunk for this file has a non-empty "
            "``summary_embedding``.  Computed against the live "
            "corpus at response time, not stored on the manifest."
        ),
    )


class AdminIndexLimits(BaseModel):
    """Explicit limitations of the current index state.

    Always surfaces ``current_process_state_only=True`` in v1 — the
    admin overview aggregates the in-memory manifest, watch
    service, and job service and does not persist any of this
    state across process restarts.  The CMS must communicate this
    limitation in the UI so operators do not interpret an empty
    overview as a clean durable manifest.
    """

    current_process_state_only: bool = Field(
        default=True,
        description=(
            "Always ``true`` in v1.  The index overview reflects only the "
            "current server-process state and is not a durable manifest."
        ),
    )


class AdminIndexVisibilityInputs(BaseModel):
    """Raw visibility/decay inputs for the admin index overview.

    These fields let the CMS display analogous visibility concepts
    for indexed artifacts (root/file/chunk counts plus the
    generation timestamp) without the backend computing a decay
    score.  Compare with :class:`AdminIndexLimits` which surfaces
    the *limits*; this block surfaces the *inputs*.
    """

    generated_at: str = Field(
        description="ISO 8601 timestamp when the overview was generated."
    )
    root_count: int = Field(
        ge=0,
        description=(
            "Number of roots currently tracked (mirrors ``len(roots)``)."
        ),
    )
    file_count: int = Field(
        ge=0,
        description=(
            "Number of files currently tracked by the manifest, "
            "falling back to the corpus count when the manifest is "
            "empty.  This is the *current-process* file count — not "
            "a durable history."
        ),
    )
    chunk_count: int = Field(
        ge=0,
        description=(
            "Number of chunks currently stored in the corpus, "
            "falling back to the sum of per-root chunk counts when "
            "the corpus status is unavailable."
        ),
    )
...
class AdminIndexOverviewResponse(BaseModel):
    ...
    Aggregates the current server-process state from the in-memory
    manifest, the live watch service, the live corpus, and the
    background-job queue.  The ``limits`` field carries the
    ``current_process_state_only=True`` signal so the CMS can
    display the data as a live view rather than historical state.
    """

    roots: list[AdminIndexRootInfo] = Field(
        description="Currently tracked index roots.",
    )
    jobs: list[AdminIndexJobInfo] = Field(
        description="Recent background indexing jobs (newest first, capped at 20).",
    )
    files: list[AdminIndexFileInfo] = Field(
        description="Currently tracked indexed files (one row per ``(root, file_path)`` pair).",
    )
    limits: AdminIndexLimits = Field(
        description="Explicit state-limitation signals.",
    )
    visibility_inputs: AdminIndexVisibilityInputs = Field(
        description="Raw fields for CMS visibility/decay display.",
    )
